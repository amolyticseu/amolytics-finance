...
import type { PayoutStatusSlice } from "@/lib/salaries/presentation"
import { formatEur } from "@/lib/format"

type PayoutStatusChartProps = {
  data: PayoutStatusSlice[]
}

export function PayoutStatusChart({ data }: PayoutStatusChartProps) {
  if (data.length === 0) {
    return (
      <p className="text-sm text-af-text-secondary">
        No active salary lines to chart in the current register.
      </p>
    )
  }
